...
class ScreenGuidedTraining:
    ...
    def launch_training(
        ...
        _SGTUI(
            ...
            time_between_reps=time_between_reps,
            randomize=randomize,
...
class _SGTUI:
    ...
    def _intialize_UI(self):
        ...
        Label(form_frame, text="Num Reps:", font=label_font).grid(
            row=0, column=0, sticky=W, padx=(0, 10), pady=(10, 5)
        )
        self._create_text_input(1, 0, 1, self.num_reps, form_frame)
        Label(form_frame, text="Time Per Rep:", font=label_font).grid(
            row=0, column=1, sticky=W, padx=(0, 0), pady=(10, 5)
        )
        self._create_text_input(1, 1, 1, self.rep_time, form_frame)
        Label(form_frame, text="Time Between Rep:", font=label_font).grid(
            row=0, column=2, sticky=W, padx=(0, 0), pady=(10, 5)
        )
        self._create_text_input(1, 2, 1, self.time_between_reps, form_frame)
        Label(form_frame, text="Input Folder:", font=label_font).grid(
            row=2, column=0, columnspan=3, sticky=W, pady=(10, 5)
        )
        self._create_text_input(3, 0, 3, self.rep_folder, form_frame)
        Label(form_frame, text="Output Folder:", font=label_font).grid(
            row=4, column=0, columnspan=3, sticky=W, pady=(10, 5)
        )
        ...
        Checkbutton(
            ...
            onvalue=True,
            offvalue=False,
        ).grid(row=6, column=0, pady=(20, 10), padx=(10, 10))
        Checkbutton(
            form_frame,
            text="Continuous",
            font=label_font,
            ...
            onvalue=True,
            offvalue=False,
        ).grid(row=6, column=1, pady=(20, 10), padx=(10, 10))
        Checkbutton(
            form_frame,
            text="GIFs",
            font=label_font,
            ...
            onvalue=True,
            offvalue=False,
        ).grid(row=6, column=2, pady=(20, 10), padx=(10, 10))
        self.start_training_button = Button(
            form_frame,
            text="Start Training",
            font=("Arial", 14),
            command=self._create_data_recording_screen,
        )
        self.start_training_button.grid(row=7, column=0, columnspan=3, pady=(10, 5))
        Button(
            form_frame, text="Visualize", font=("Arial", 14), command=self._visualize
        ).grid(row=8, column=0, columnspan=3, pady=(10, 5))
        self.error_label = Label(
            form_frame,
            text="Online Data Handler not reading data...",
            font=("Arial", 12),
            bg="#FFFFFF",
            fg="#FF0000",
        )
    ...
    def _create_text_input(self, row, col, col_span, default_text, frame):
        ...
        entry.grid(
            row=row, column=col, columnspan=col_span, sticky=N + S + W + E, padx=(0, 10)
        )
        return entry
    ...
    def _collect_data(self):
        self.rep_label["text"] = (
            "Rep " + str(self.rep_number + 1) + " of " + str(self.num_reps.get())
        )
        if self.rep_number < int(self.num_reps.get()):
            ...
            while file_index < len(self.inputs):
                ...
                for val in range(0, 2):
                    ...
                    if val == 0:
                        ...
                        self.image_label.load(
                            image_file,
                            True,
                            self.photo_width,
                            self.photo_width,
                            self.rep_time.get(),
                            self.time_between_reps.get(),
                        )
                    else:
                        self.image_label.unload()
                        self.image_label.load(
                    ...
                    if val != 0:
                        emg_data[self.og_inputs.index(file)] = (
                            self.data_handler.get_data()
                        )
                        imu_data[self.og_inputs.index(file)] = (
                            self.data_handler.get_imu_data()
                        )
                        other_data[self.og_inputs.index(file)] = (
                            self.data_handler.get_other_data()
                        )
                    if val == 1 and self.wait and file != self.inputs[-1]:
                        ...
                        self.window.bind("<space>", lambda _: self._next())
                        redo_gest = Button(
                            self.window,
                            text="Redo",
                            font=("Arial", 12),
            ...
            self.window.bind("<Return>", lambda _: self._next_rep())
        ...
        for c in emg_data.keys():
            ...
            if other_data[c]:  # Only write if there is other data
                for k in other_data[c].keys():
                    with open(
                        other_file + k + ".csv", "w", newline="", encoding="utf-8"
                    ) as file:
        ...
        try:
            for i in count(1):
                if gray:
                    frames.append(
                        ImageTk.PhotoImage(
                            im.copy().convert("L").resize((width, height))
                        )
                    )
                else:
